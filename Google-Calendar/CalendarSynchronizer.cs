...
public class CalendarSynchronizer
{
    ...
    private bool NeedsUpdate(Zadanie task, Google.Apis.Calendar.v3.Data.Event ev)
    {
        ...
        // SprawdŸ deadline
        var evDeadline = GoogleCalendarReader.GetDeadline(ev.Start?.DateTimeDateTimeOffset, ev.Start.Date);
    ...
    private void UpdateTask(Zadanie task, Google.Apis.Calendar.v3.Data.Event ev)
    {
        if (ev.Status != "cancelled")
        {
            ...
            task.Deadline = GoogleCalendarReader.GetDeadline(ev.Start?.DateTimeDateTimeOffset, ev.Start.Date);
